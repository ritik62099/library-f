...
export default function AttendanceForm() {
  ...
  const adminId = urlParams.get("admin"); // ✅ QR admin param
    ...
    try {
      ...
      const found = students.data.find(
        (s) =>
          s.rollNo.toLowerCase() === form.rollNo.toLowerCase() &&
          s.name.toLowerCase() === form.name.toLowerCase() &&
          s.admin === adminId // filter by admin
      );

      if (!found) {
        setMessage({ text: "❌ Student not found for this admin.", type: "error" });
      } else {
        await API.post("/attendance/mark", { studentId: found._id, admin: adminId });
